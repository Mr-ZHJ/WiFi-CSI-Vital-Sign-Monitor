%% PButterWorthFilterM.m —— 动态场景相位 Butterworth 带通滤波（心跳频段）
% 功能：对去均值后的动态场景相位信号做 FFT 频域分析，设计 3~5 Hz 的
%       Butterworth 带通滤波器（心跳谐波频段），提取心跳分量。
%
% 输入：res_csi（Hampel + 小波去噪后的相位序列）
% 输出：y（带通滤波后的心跳波形）
%%

% 第一步：去均值 + FFT 频域分析
res_csi2 = res_csi2 - mean(res_csi);            % 去均值
...
% buttord：计算最小滤波器阶数；butter：生成模拟带通滤波器系数
