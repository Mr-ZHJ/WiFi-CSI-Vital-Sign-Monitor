%% AHampelFilter.m —— 对幅度比序列执行 Hampel 滤波
% 功能：对幅度比序列 raw_csi_amplitude 执行 Hampel 滤波剔除异常值，
%       绘制原始曲线并标记被判为异常的样本点（红色方块）。
%
% 输入：raw_csi_amplitude（由 GetAmplitudeRatio.m 生成的幅度比序列）
...
time = 0.05:0.05:50;   % 时间轴：每包 0.05s
